...
Вернёмся к решению. Наш код преобразования из числа в строку можно слегка сократить, передав математическое выражение `first_car_price + second_car_price` напрямую в качестве аргумента команде `str`.
...
Остановимся на вопросе тестирования. В задании я просил тебя сразу протестировать первую же добавленную фичу (например, умножение), и это не лишено смысла. Чем короче путь от внесения изменений в код до получения обратной связи, тем лучше. В данном случае под обратной связью я понимаю проверку того, что изменения в коде действительно привели к желаемому результату.
...
Прежде всего, проведём так называемое *регрессионное тестирование*: убедимся, что мы ничего не сломали из уже имеющегося. Проверь операции сложения и вычитания. Начинать тестирование логичней с наиболее вероятных сценариев штатной работы приложения, так называемых *happy paths*. В нашем случае мы можем попросить программу сложить два небольших положительных числа, например `3` и `2`, введя код операции `+`. Мы ожидаем сообщения `Result of addition: 5` в качестве ответа. Затем введём те же числа с кодом операции вычитания (`-`). Упс! Мы обнаружили первый баг. Очевидно, что ответом должно быть `Result of subtraction: 1`, а программа выдала `Result of addition: 1`!

Это несложный баг, который ты, уверен, уже знаешь, как исправить, если не исправила до этого. Однако давай зафиксируем его по всем правилам. Если бы ты работала настоящим программистом или тестировщиком, ты была бы должна создать запись в [системе отслеживания ошибок](https://ru.wikipedia.org/wiki/%D0%A1%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D0%B0_%D0%BE%D1%82%D1%81%D0%BB%D0%B5%D0%B6%D0%B8%D0%B2%D0%B0%D0%BD%D0%B8%D1%8F_%D0%BE%D1%88%D0%B8%D0%B1%D0%BE%D0%BA) (баг трекере). Их великое множество, и нет цели учить тебя пользоваться каким-либо из них. Важна суть записи, которую ты должна была бы там сделать. Опуская детали и корпоративные стандарты, при описании бага необходимо указать три вещи:
1. шаги воспроизведения (как обнаружить баг),
...
Заключение об ожидаемом поведении программы в нашем примере мы делаем основываясь на здравом смысле. В корпоративной же среде требования к поведению программы могут быть прописаны в *спецификациях* и *тестовых сценариях*. Если наблюдаемое поведение нарушает какой-то пункт из "спеки" или сценария, нужно его указать. Разумеется, зравый смысл в корпоративной среде тоже никто не отменял (~~но это не точно~~). Также можно прикладывать скриншоты в качестве свидетельства.

Шаги воспроизведения желательно описывать так, чтобы твой коллега, не полностью погружённый в контекст задачи, или ты сама, вернувшаяся к работе над программой полгода спустя, легко могли эти шаги выполнить.

Сказанное выглядит весьма занудно, но в процессе разработки программы (который включает себя тестирование!) важна кристальная ясность мышления. А эти действия, как и другие приёмчики, о которых я уже рассказал и расскажу дальше, помогают лучше организовать процесс мышления. Кроме того, программирование - давно не удел одиночек и требует эффективной коммуниакции в команде. Умение описывать неисправность - ключевой навык общения ИТ-специалиста. Пусть ты и не будешь работать в ИТ - это поможет почувствовать дух профессии.

Вернёмся к тестированию калькулятора. Допустим, что баг мы уже исправили, и теперь продолжим регрессионное тестирование сложения и вычитания, но с менее тривиальными входными данными. Тестовые сценарии зачастую однообразны, и многие из них можно свести в табличку. В нашем случае табличка может выглядеть очевидным образом:
...
Список сценариев можно дополнять практически бесконечно: все комбинации перебрать невозможно. Наша цель - перебрать ключевые случаи, а дальше пополнять список в ходе использования программы реальными пользователями, когда они сообщают о какой-то ошибке, а разработчики подтверждают наличие этой ошибки и устраняют её. Очень хорошо, когда каждый "багофикс" идёт вкупе хотя бы с одним проверочным тестом. Этот тест должен "падать" до исправления, и успешно проходить после.

Написание тестовых сценариев даже для простого учебного калькулятора отняло у меня довольно много времени. Очевидно, я не буду требовать столь исчерпывающего тестирования каждой учебной поделки. Но в данном упражнении я дал общее представление о предмете. А ты помни о большом количестве разных вариантов, в том числе неочевидных, и старайся развивать свою программисткую интуицию: нужно не восхищаться очередным своим творением, а критически относиться к нему, пытаясь найти в нём баги и уязвимости.

Честно пройдя по списку пару раз, проверив вручную все сценарии, ты, уверен, захочешь как-то автоматизировать процесс. Ведь у нас даже есть табличка, её, вероятно, можно как-то запрограммировать; у компьютера проход по такой табличке займёт мгновение! Да, это можно сделать. Позже мы научимся писать автоматические тесты. TODO!!!!!!!!!!! А написание тестов на основе таких табличек называется [data-driven или параметризированным тестированием](https://en.wikipedia.org/wiki/Data-driven_testing). Оно не всегда применимо, но когда хотя бы часть сценариев удаётся свести к табличке, выделив общую повторяющуюся часть, это делает тесты гораздо более выразительными и доступными для понимания.

Обрати внимание, когда мы нашли ошибку в сообщении с результатом вычитания, я назвал её *первым* багом. Я *не* расскажу тебе о втором и других багах в программе, потому что сам не знаю, есть ли они. Дело в том, что тестирование может показать только *наличие* багов, но не их *отсутствие*.
