...
И ещё. Когда пользователь вводит что-то кроме знака операции `+`, `-`, `*` или `/`, сделай так, чтобы программа выдавала сообщение `I don't understand you!`. Так наша программа будет предсказуемей с точки зрения пользователя. UX (*User eXperience*) - опыт и впечателения пользователя от взаимодействия с программой - тоже очень важная штука. Чем меньше неоднозначностей и преположений, о которых пользователь может не знать, тем лучше.
