...
Давай теперь начнём новую тему. Наш калькулятор замечательно складывает числа. Но что делать, если пользователю ещё нужно и вычитание? Конечно, в такой простой программе на Питоне можно прямо в самом коде поменять плюс на минус и получить новую "фичу". Но программы бывают огромными. Кроме того, как мы помним, не все программы распространяются в виде исходного кода на Питоне. Короче, мы хотим, чтобы наша программа спрашивала пользователя, хочет ли он складывать или вычитать. Как-то так:
...
Ведь одиночный символ равенства у нас уже занят под присвоение значений переменным! А таким "нестандартным" символом равенства мы даём компилятору понять, что мы не сохраняем что-то в переменную, а сравниваем её значение с чем-то, что справа от двойного равно, то есть с плюсом в нашем случае. Такая странность присутствует не только в Питоне, но и других языках, однако, не во всех.

Следующие две строчки, идущие после двоеточия, довольно очевидны, за исключением того, что имеют отступ от края. И этот отступ в Питоне играет важную роль. Во-первых, даже если бы такой отступ был необязателен, отметим, что он сам по себе улучшает читаемость программы: мы сразу видим, что есть два блока, два режима выполнения - первый для сложения, второй для вычитания. Сравни с неправильным вариантом без отступов:
