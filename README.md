...
В случае с конструкцией `elif` мы рассмотрели, как можно обойтись без неё. Подумай, а можно ли было в задаче 1б обойтись без `else`, только с помощью `if` (мы помним, что `if` может идти без секции `else`)?
