...
5. Кроме того,  мы интуитивно, без строгих определений ввели понятие "логического и" и использовали соответствующий оператор (`and`) в условии цикла:
   ...
   ```
   Далее мы рассмотрим ещё несколько инструментов, которые позволят нам составлять логические условия любой сложности.
...
7. Кроме того, простоте очень способствует отсутствие в программе так называемого мёртвого кода - тех команд, которые ни при каких условиях не могут быть выполнены. В нашем примере калькулятора мы идентифицировали такой кусок:
   ```python
       ...
       print("I don't understand you!")
   ```
Эта команда `print` никогда не выполнялась бы, потому что перед ней мы добавили код, который не позволяет пользователю ввести некорректный код операции (точнее, если пользователь вводил какую-то белиберду, инетпретатор перезапрашивал код операции).
