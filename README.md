...
# Приступим. Что значит программировать
...
Обрати внимание, вставленный кусок выделен отступами, так как он принадлежит секции `else` условного оператора `if t > 100`! Сравни с тем, что было до этого:
...
Продолжим в том же духе, покрывая оставшиеся варианты значений `t` и наращивая уровни вложенности. Имеющийся `print("Something else")` заменим на обработку `if t > 0 ... else ....`:
...
Довольно простое задание, его можно делать для того, чтобы просто вспомнить какие-то подзабытые моменты из пройденного. Именно поэтому в разборе решения я ещё раз довольно подробно пройдусь по особенностям синтаксиса Питона.
...
Закономерно, что код очень похож на решение задачи 1б, за исключением секции `else`:
